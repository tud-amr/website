...
> [!WARNING]  
> We still need to fix an issue with the custom domain, so for now the webiste is accessible from here https://github.com/tud-amr/website. NEVERTHELES, use the new webiste template already. The domain https://www.autonomousrobots.nl/ will still show the old website until this is fixed.
